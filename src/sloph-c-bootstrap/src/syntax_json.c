...
SlophStatus sloph_syntax_to_json(SlophContext*c,const SlophSyntaxModule*m,SlophSyntaxText*out){yyjson_mut_doc*d;yyjson_mut_val*r;char*raw,*copy;size_t n;const SlophAllocator*a;if(c==NULL||m==NULL||out==NULL)return SLOPH_STATUS_INVALID_ARGUMENT;out->data=NULL;out->length=0;d=yyjson_mut_doc_new(NULL);if(!d)return SLOPH_STATUS_OUT_OF_MEMORY;r=yyjson_mut_obj(d);yyjson_mut_obj_add_val(d,r,"module",module_node(d,m));yyjson_mut_obj_add_str(d,r,"schema","sloph.syntax");yyjson_mut_obj_add_uint(d,r,"version",m->version);yyjson_mut_doc_set_root(d,r);raw=yyjson_mut_write(d,0u,&n);yyjson_mut_doc_free(d);if(!raw)return SLOPH_STATUS_OUT_OF_MEMORY;if(n+1u>sloph_context_limits(c)->output_bytes){free(raw);return sloph_syntax_diagnostic(c,"syntax.json.limit_exceeded","json","output_bytes limit exceeded",SLOPH_UNKNOWN_SPAN,SLOPH_STATUS_LIMIT_EXCEEDED);}a=sloph_context_allocator(c);copy=a->allocate(a->user_data,n+2u);if(!copy){free(raw);return SLOPH_STATUS_OUT_OF_MEMORY;}memcpy(copy,raw,n);copy[n]='\n';copy[n+1u]=0;free(raw);out->data=copy;out->length=n+1u;return SLOPH_STATUS_OK;}
