...
static SlophSyntaxExpr *parse_atom(Parser *p){Token*t=&p->tokens[p->index];size_t start=t->start;SlophSyntaxExpr*e;SlophSpan sp;
    ...
    if(p->module->version==1u&&t->length>=2u&&t->text[0]=='\"'){Vector bytes={0};size_t i=1u;take(p,NULL);while(i+1u<t->length){unsigned char c=(unsigned char)t->text[i++];if(c=='\\'){char q=t->text[i++];if(q=='x'&&i+1u<t->length){if(!isxdigit((unsigned char)t->text[i])||!isxdigit((unsigned char)t->text[i+1u])){error(p,"syntax.parse.unexpected_token","unknown byte escape",span_at(t->start,t->end));break;}c=(unsigned char)((hex_value(t->text[i])<<4)|hex_value(t->text[i+1u]));i+=2u;}else{switch(q){case'n':c=10;break;case'r':c=13;break;case't':c=9;break;case'0':c=0;break;case'\\':c=92;break;case'\"':c=34;break;default:error(p,"syntax.parse.unexpected_token","unknown byte escape",span_at(t->start,t->end));}}}vector_push(p,&bytes,&c,1u);}e=new_expr(p,SLOPH_SYNTAX_EXPR_BYTES,span_at(t->start,t->end));if(e){e->as.bytes.length=bytes.count;e->as.bytes.data=vector_finish(p,&bytes,alignof(unsigned char));}return e;}
